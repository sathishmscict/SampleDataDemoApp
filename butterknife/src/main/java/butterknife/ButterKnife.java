...
public final class ButterKnife {
    ...
    private static Unbinder createBinding(@NonNull Object target, @NonNull View source) {
        ...
        if (BuildConfig.DEBUG) Log.d(TAG, "Looking up binding for " + targetClass.getName());
    ...
    private static Constructor<? extends Unbinder> findBindingConstructorForClass(Class<?> cls) {
        ...
        if (bindingCtor != null) {
            if (BuildConfig.DEBUG) Log.d(TAG, "HIT: Cached in binding map.");
            return bindingCtor;
        }
        String clsName = cls.getName();
        if (clsName.startsWith("android.") || clsName.startsWith("java.")) {
            if (BuildConfig.DEBUG) Log.d(TAG, "MISS: Reached framework class. Abandoning search.");
            return null;
        }
        try {
            ...
            if (BuildConfig.DEBUG) Log.d(TAG, "HIT: Loaded binding class and constructor.");
